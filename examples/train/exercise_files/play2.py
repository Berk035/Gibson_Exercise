...
def play(env, transpose=True, zoom=None, init=None, callback=None, keys_to_action=None):
...
#    assert len(obs_s.shape) == 2 or (len(obs_s.shape) == 3 and obs_s.shape[2] in [1,3])
    ...
    while running:
        ...
        if len(pressed_keys) == 0:
            ...
            record_num += 1
            #print("Play mode----->record time:%f, record num:%i" % (record_total,record_num))
        ...
        if callback is not None:
            ...
            #init(callback=init, horizon_timesteps=300, plot_names=["reward"], obs_t=prev_obs, obs_tp1=obs,
            #     action=action, rew=rew, done=env_done, info=info)
            #callback(prev_obs, obs, action, rew, env_done, info)
            #c = callback(horizon_timesteps=300, plot_names="Reward")
            #c(obs_t=prev_obs, obs_tp1=obs, action=action, rew=rew, done=env_done, info=info)
...
class PlayPlot(object):
    ...
    def callback(self, obs_t, obs_tp1, action, rew, done, info):
        points = self.data_callback(self,rew)
        for points, data_series in zip(points, self.data):
